...
# Custom CSS เพื่อให้ UI ดูสวยงามแบบ Dashboard จริง
st.markdown("""
<style>
    /* พื้นหลังหลัก */
    ...
    if email_cols:
        def mask_email(email):
            ...
            if '@' in email:
                ...
                return f"{parts}***@{parts}"
            return "-"
        df[email_cols] = df[email_cols].apply(mask_email)
...
with st.sidebar:
    st.image(
        "https://upload.wikimedia.org/wikipedia/commons/thumb/5/53/Wikimedia-logo.png/240px-Wikimedia-logo.png",
        width=80
    )
...
if sel_type  != 'ทั้งหมด' and user_type_col: dff = dff[dff[user_type_col] == sel_type]
if sel_dept  != 'ทั้งหมด' and dept_col:      dff = dff[dff[dept_col]      == sel_dept]
if sel_status!= 'ทั้งหมด' and status_col:    dff = dff[dff[status_col]    == sel_status]
if sel_return!= 'ทั้งหมด' and return_col:    dff = dff[dff[return_col]    == sel_return]

if date_range and 'Timestamp' in dff.columns and len(date_range) == 2:
    start_dt = pd.Timestamp(date_range)
    end_dt   = pd.Timestamp(date_range) + pd.Timedelta(days=1)
...
<div style='background: linear-gradient(135deg,#667eea,#764ba2);
            padding:30px; border-radius:20px; margin-bottom:20px; color:white; text-align:center;'>
    <h1 style='margin:0; font-size:2.2rem;'>🧪 ระบบรายงานสถิติการขอใช้เครื่องมือวิทยาศาสตร์</h1>
...
# ============================================================
# 8. Section A — KPI Scorecard (ตัวเลขตัวใหญ่)
# ============================================================
...
with kpi2:
    if 'Timestamp' in dff.columns:
        this_m_count = len(dff[dff['Timestamp'].dt.to_period('M') == this_month])
    else:
        this_m_count = 0
    st.markdown(f"""
...
with kpi3:
    if status_col:
        ...
        pending = 0
    st.markdown(f"""
    <div class='metric-card' style='background:linear-gradient(135deg,#f6d365,#fda085);'>
        <h2>{pending}</h2>
        <p>⏳ รออนุมัติ</p>
    </div>""", unsafe_allow_html=True)
...
def generate_ai_insight(dff, status_col, return_col, user_type_col, dept_col):
    """สร้างข้อความ Insight อัตโนมัติจากข้อมูล"""
    insights = []
    warnings = []
    ...
    # 3. คณะ/หน่วยงานที่ขอใช้มากที่สุด
    if dept_col:
    ...
    if 'Hour' in dff.columns:
        top_hour = dff['Hour'].value_counts().idxmax()
        insights.append(
            f"⏰ ช่วงเวลาที่มีการส่งคำขอมากที่สุดคือ **{top_hour}:00 - {top_hour+1}:00 น.**"
        )

    # 5. วันในสัปดาห์ยอดนิยม
    if 'DayOfWeek' in dff.columns:
        day_map = {
            'Monday':'วันจันทร์','Tuesday':'วันอังคาร','Wednesday':'วันพุธ',
            'Thursday':'วันพฤหัสบดี','Friday':'วันศุกร์',
            'Saturday':'วันเสาร์','Sunday':'วันอาทิตย์'
        }
        top_day = dff['DayOfWeek'].value_counts().idxmax()
        insights.append(
            f"📅 **{day_map.get(top_day, top_day)}** คือวันที่มีคำขอบ่อยที่สุดในสัปดาห์"
        )
    ...
    if return_col:
        ...
        if rate < 50:
            ...
            insights.append(f"✅ อัตราการคืนอุปกรณ์อยู่ที่ **{rate:.1f}%** — อยู่ในเกณฑ์ดี")
...
# ============================================================
# 10. Section C — Monthly Trend (กราฟเส้นรายเดือน)
# ============================================================
...
if 'Month_dt' in dff.columns and not dff['Month_dt'].isna().all():
    ...
    if user_type_col:
        ...
        monthly_type['Month_Label'] = monthly_type['Month_dt'].dt.strftime('%b %Y')
...
# ============================================================
# 11. Section D — Top 5 Tools / Locations
# ============================================================
...
with col_t1:
    if tool_col:
        ...
        top_tools.columns = ['เครื่องมือ', 'จำนวน']
        fig_tools = px.bar(
            top_tools,
            x='จำนวน',
            y='เครื่องมือ',
            orientation='h',
            title='🔬 Top 5 เครื่องมือที่ถูกขอใช้มากที่สุด',
            color='จำนวน',
        ...
        # Fallback: ใช้คอลัมน์ประเภทผู้ใช้แทน
        if user_type_col:
            top_ut = dff[user_type_col].value_counts().head(5).reset_index()
            top_ut.columns = ['ประเภท', 'จำนวน']
            fig_ut = px.bar(
                top_ut, x='จำนวน', y='ประเภท', orientation='h',
                title='Top 5 ประเภทผู้ใช้งาน',
                color='จำนวน', color_continuous_scale='Blues', text='จำนวน'
            )
...
with col_t2:
    if location_col:
        ...
        top_locs.columns = ['สถานที่', 'จำนวน']
        fig_locs = px.bar(
            top_locs,
            x='จำนวน',
            y='สถานที่',
            orientation='h',
            title='🏠 Top 5 สถานที่/โรงเรือนที่ถูกขอใช้มากที่สุด',
            color='จำนวน',
            color_continuous_scale='Oranges',
            text='จำนวน'
        )
        fig_locs.update_traces(textposition='outside')
        ...
        # Fallback: ใช้คณะแทน
        if dept_col:
            top_dept = dff[dept_col].value_counts().head(5).reset_index()
            top_dept.columns = ['คณะ/หน่วยงาน', 'จำนวน']
            fig_dept2 = px.bar(
                top_dept, x='จำนวน', y='คณะ/หน่วยงาน', orientation='h',
                title='Top 5 คณะ/หน่วยงานที่ขอใช้บริการมากที่สุด',
                color='จำนวน', color_continuous_scale='Oranges', text='จำนวน'
            )
            fig_dept2.update_traces(textposition='outside')
...
# ============================================================
# 12. Section E — Pie Charts (สัดส่วน)
# ============================================================
...
# ============================================================
# 13. Section F — Heatmap รายวันในสัปดาห์ vs ช่วงเวลา
# ============================================================
...
# ============================================================
# 14. Section G — ตารางรายการเกินกำหนด (Conditional Formatting)
# ============================================================
...
if date_end_col and return_col:
    ...
    if not overdue_df.empty:
        ...
        overdue_cols = ['ชื่อ-สกุล', date_end_col, return_col,
        ...
        st.dataframe(
            overdue_df[valid_oc].sort_values('เกินกำหนด (วัน)', ascending=False),
...
# ============================================================
# 15. Section H — ตารางรายละเอียดทั้งหมด (พร้อม Search)
# ============================================================
...
email_display = email_cols_for_display if email_cols_for_display else None

display_cols = [
    'Timestamp', 'ชื่อ-สกุล', user_type_col, dept_col,
    purpose_col, 'เบอร์โทรศัพท์เพื่อติดต่อ',
    email_display, status_col, return_col
]
